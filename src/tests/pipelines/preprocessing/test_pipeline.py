...
"""
from cartola_estimator.pipelines.preprocessing.nodes import (
    add_match_info,
    create_target_data,
...
def test_add_match_info(cartola_raw_data, cb_data):
    ...
    assert (
        cartola_info.mando
        == ["mandante", "visitante", "mandante", "mandante", "mandante", "visitante"]
    ).all()
...
def test_create_hist_features(cartola_data, columns, stats_dict, id_cols):
    ...
    assert (
        hist_features[hist_features.rodada == 2]["G_atletas_posicao_id"].mean() == 0.5
    )
