...
def add_match_info(
    ...
    cb_data: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    cartola_data['clube'] = cartola_data['clube'].replace({'Atletico-PR': 'Athletico-PR'})
    cartola_data = cartola_data.drop_duplicates(["rodada", "ano","atletas_slug", "clube"])
    ...
    mando = pd.melt(cb_data[["rodada", "ano", "mandante", "visitante", "vencedor"]],
    ...
    mando["oponente"] = np.where(mando.mando == "mandante", mando.visitante, mando.mandante)
    mando["resultado"] = np.where(mando.clube == mando.vencedor, "V", mando.vencedor)
    mando["resultado"] = np.where(mando.resultado == mando.oponente, "D", mando.resultado)
...
def create_target_data(
    data: pd.DataFrame,
    stats_dict: Dict[str, Any],) -> pd.DataFrame:
    ...
    target["time_idx"] = ((target["ano"] - target["ano"].min()) \
                           * target["rodada"].max() + target["rodada"])
    return target
